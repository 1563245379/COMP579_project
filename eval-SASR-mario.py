...
def parse_args():
    ...
    parser.add_argument("--model-dir", type=str, default="./duel-dqn-mario/",
...
def load_models(env, args, device):
    ...
    actor = SACActorDiscrete(env, new_version=False).to(device)
    qf_1 = QNetworkDiscrete(env, new_version=False).to(device)
    qf_2 = QNetworkDiscrete(env, new_version=False).to(device)
